...
TEST_F(RocksDBMetricsTest, DumpAllStats) {
    ...
    ASSERT_TRUE(j.contains("main")) << "Response JSON does not contain 'main'";
    ASSERT_TRUE(j.contains("histogram")) << "Response JSON does not contain 'histogram'";
    ASSERT_TRUE(j.contains("columns")) << "Response JSON does not contain 'columns'";

    // Check main metrics for non-zero values after operations
    if (j["main"].contains("rocksdb.bytes.read")) {
        EXPECT_GT(j["main"]["rocksdb.bytes.read"], 0) << "Bytes read should be greater than 0 after reads.";
    }
    if (j["main"].contains("rocksdb.bytes.written")) {
        EXPECT_GT(j["main"]["rocksdb.bytes.written"], 0) << "Bytes written should be greater than 0 after writes.";
    }

    // Verify histogram metrics, particularly for latency and count of read/write operations
    if (j["histogram"].contains("rocksdb.db.get.micros")) {
        EXPECT_GT(j["histogram"]["rocksdb.db.get.micros"]["count"], 0) << "Get micros count should be greater than 0 after reads.";
        EXPECT_GT(j["histogram"]["rocksdb.db.get.micros"]["avg"], 0) << "Get micros avg should be greater than 0.";
    }

    if (j["histogram"].contains("rocksdb.db.write.micros")) {
        EXPECT_GT(j["histogram"]["rocksdb.db.write.micros"]["count"], 0) << "Write micros count should be greater than 0 after writes.";
        EXPECT_GT(j["histogram"]["rocksdb.db.write.micros"]["avg"], 0) << "Write micros avg should be greater than 0.";
    }

    // Check SST file metrics in columns section
    if (j["columns"].contains("all_columns")) {
        EXPECT_GE(j["columns"]["all_columns"]["sum"]["NumFiles"], 1) << "Number of SST files should be at least 1.";
    }

    // Verify block cache metrics
    if (j["main"].contains("rocksdb.block.cache.hit")) {
        EXPECT_GT(j["main"]["rocksdb.block.cache.hit"], 0) << "Block cache hits should be greater than 0.";
    }
    if (j["main"].contains("rocksdb.block.cache.miss")) {
        EXPECT_GE(j["main"]["rocksdb.block.cache.miss"], 0) << "Block cache misses should be >= 0.";
    }
}
